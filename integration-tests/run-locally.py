...
def start_dockerized_apps():
    if is_docker_container_running('postgres') and is_docker_container_running(
            'ringring') and is_docker_container_running('invoices') and len(sys.argv) == 1:
...
def is_docker_container_running(container_name):
    return 'true' == subprocess.run("docker inspect -f '{{.State.Running}}' " + container_name, shell=True,
...
if __name__ == "__main__":
    ...
    os.environ["TESTING_HOST"] = "host.docker.internal"
